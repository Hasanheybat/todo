...
export default function DashboardPage() {
  ...
  function loadData() {
    Promise.all([
      ...
      setTasks(t); setUsers(u); setDepartments(d); setTodoTasks(todos.filter((tk: any) => !tk.isCompleted))
  ...
  const filteredTodos = todoStatusFilter === 'ALL'
    ? incompleteTodos
    : incompleteTodos.filter(t => (t.todoStatus || 'WAITING') === todoStatusFilter)
  ...
  return (
    <div className="pb-10">
      {/* Başlıq — Slate: yüngül, Default: böyük */}
      ...
      <GlassFilterBar
        ...
        statusTabs={viewFilter === 'todo' ? [
          ...
          { key: 'WAITING',     label: 'Gözləyir',  dot: '#64748B', count: incompleteTodos.filter(t => (t.todoStatus||'WAITING')==='WAITING').length },
          { key: 'IN_PROGRESS', label: 'Davam edir',dot: '#3B82F6', count: incompleteTodos.filter(t => (t.todoStatus||'WAITING')==='IN_PROGRESS').length },
          { key: 'DONE',        label: 'Tamamlandı',dot: '#10B981', count: incompleteTodos.filter(t => (t.todoStatus||'WAITING')==='DONE').length },
          { key: 'CANCELLED',   label: 'İptal',     dot: '#EF4444', count: incompleteTodos.filter(t => (t.todoStatus||'WAITING')==='CANCELLED').length },
        ] : [
