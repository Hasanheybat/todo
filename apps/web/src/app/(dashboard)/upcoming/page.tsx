...
export default function UpcomingPage() {
  ...
  // Təqvim üçün birləşmiş TODO+GÖREV data (viewFilter-ə görə filtr)
  const calendarTasks = useMemo(() => {
    const gorevItems = viewFilter !== 'todo' ? displayTasks.map(t => ({
      id: t.id, type: 'gorev' as const, title: t.title, priority: t.priority,
      dueDate: t.dueDate?.split('T')[0] || '', raw: t,
    })) : []
    const todoItems = viewFilter !== 'gorev' ? incompleteTodos.map(t => ({
      id: t.id, type: 'todo' as const, title: t.content || 'Todo', priority: t.priority,
      dueDate: t.dueDate?.split('T')[0] || '', raw: t,
    })) : []
    return [...gorevItems, ...todoItems]
  }, [displayTasks, incompleteTodos, viewFilter])
              ...
              <div className="flex items-center gap-2 mb-3">
                ...
                <div className="flex rounded-md overflow-hidden border border-[var(--todoist-divider)]">
                  <button onClick={() => setTodoView('list')}
                    className={`px-2.5 py-1 text-[10px] font-bold flex items-center gap-1 transition
        ...
        <div>
          ...
          <div className="grid grid-cols-7 gap-px bg-[var(--todoist-divider)] border border-[var(--todoist-divider)] rounded-xl overflow-hidden">
            ...
            {calendarDays.map((cell, i) => {
              ...
              const isDragOver = calDragOverDate === cell.dateStr
              return (
                <div key={i}
                  className={`min-h-[72px] p-1.5 transition-all ${cell.isCurrentMonth ? (isToday ? 'bg-[#FFF5F3]' : 'bg-white') : 'bg-[var(--todoist-bg)]'}`}
                  style={isDragOver ? { outline: '2px dashed var(--todoist-red)', outlineOffset: '-2px' } : {}}
                  onDragOver={e => { e.preventDefault(); setCalDragOverDate(cell.dateStr) }}
                  onDragLeave={() => setCalDragOverDate(null)}
                  onDrop={async (e) => {
                    e.preventDefault()
                    setCalDragOverDate(null)
                    if (calDragTodoId && cell.dateStr) {
                      try {
                        await api.updateTodoistTask(calDragTodoId, { dueDate: cell.dateStr })
                        toast('📅 Tarix dəyişdirildi')
                        loadData()
                      } catch { toast.error('Xəta baş verdi') }
                      setCalDragTodoId(null)
                    } else if (calDragGorevId && cell.dateStr) {
                      try {
                        await api.updateTask(calDragGorevId, { dueDate: cell.dateStr })
                        toast('📅 Tapşırıq tarixi dəyişdirildi')
                        loadData()
                      } catch { toast.error('Xəta baş verdi') }
                      setCalDragGorevId(null)
                    }
                  }}>
                  ...
                  {cellTasks.slice(0, 2).map(task => (
                    <div key={`${task.type}-${task.id}`}
                      ...
                      draggable
                      onDragStart={() => {
                        if (task.type === 'todo') setCalDragTodoId(task.id)
                        else setCalDragGorevId(task.id)
                      }}
                      onDragEnd={() => { setCalDragTodoId(null); setCalDragGorevId(null) }}
                      className={`mt-0.5 text-[8px] px-1 py-px rounded font-semibold truncate cursor-grab active:cursor-grabbing hover:opacity-80 transition
